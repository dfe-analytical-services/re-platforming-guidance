...
In R, Markdown can be used to create html outputs with dynamic visuals and code snippets and this can be taken one step further and published via Bookdown.  This is often done to produce reports or guidance once the code has been completed. Separate .Rmd scripts will be needed which means that if you make changes to R scripts you may also need to duplicate these in .Rmd files for example.
